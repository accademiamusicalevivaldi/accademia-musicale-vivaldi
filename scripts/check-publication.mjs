...
try{
 ...
 execFileSync('npm',['run','build:upload'],{stdio:'pipe',env:{...process.env,ASTRO_TELEMETRY_DISABLED:'1',WRANGLER_SEND_METRICS:'false'}});
 ...
 assert.ok(fs.readFileSync('dist/gallerie/qa-album/index.html','utf8').includes('photo-link'));assert.ok(fs.readFileSync('dist/documenti/index.html','utf8').includes('123456789abc.pdf'));assert.ok(fs.readFileSync('dist/index.html','utf8').includes('Maestra Vanessa Visentin'));assert.ok(fs.statSync('dist/_worker.js').isDirectory());execFileSync('node',['--check','dist/_worker.js/index.js'],{stdio:'pipe'});console.log('Verificate pagine articolo, album, PDF, esclusione bozze e Worker.');
}finally{for(const f of files)if(fs.existsSync(f))fs.unlinkSync(f);execFileSync('npm',['run','build:upload'],{stdio:'pipe',env:{...process.env,ASTRO_TELEMETRY_DISABLED:'1',WRANGLER_SEND_METRICS:'false'}});console.log('File di prova rimossi, sito finale ricostruito.');}
